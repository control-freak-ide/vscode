...
export class SimpleSource implements IReplElementSource {
	...
	constructor(public readonly uri: uri, public readonly lineNumber: number, public readonly column: number) {
		this.source = {
			...
			uri,
			openInEditor: (editorService: IWorkbenchEditorService, selection: IRange, preserveFocus?: boolean, sideBySide?: boolean) => {
				return editorService.openEditor({
					resource: this.uri,
					options: {
						preserveFocus,
						selection: { startColumn: this.column, startLineNumber: this.lineNumber } as IRange,
